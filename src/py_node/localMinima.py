...
############## This is part for you: Until line 330 #####################
    ...
    def scan_cb(self, msg):
        ...
        if corners[0].size == 0 and corners[1].size == 0:
    ...
    def detect_corners(self, pos, angles, ranges, angle_increment):
        ...
        dist_expected = ranges_sub * np.tan(angle_increment * step)


        left_mask = angles_sub <= np.pi

        # print("####### Left Mask #####")
        # print(left_mask)
        # print(angles_sub[left_mask])
        # print([len(angles_sub[left_mask]), len(angles_sub[~left_mask])])
        # print("####### Left Mask #####")

        left_angles = angles_sub[left_mask]
        left_ranges = ranges_sub[left_mask]
        left_pos = pos_sub[:, left_mask]

        left_min_dist = dist_expected[left_mask]
        left_min_dist = np.maximum(2*left_min_dist[:-1], 1.0)

        # right_mask = ~left_mask
        # right_mask[0] = True

        right_angles = angles_sub[~left_mask]
        right_angles = (right_angles + np.pi)%(2 * np.pi) - np.pi
        right_angles = np.append(angles_sub[0], right_angles[::-1])
        right_ranges =  np.append(angles_sub[0], ranges_sub[~left_mask][::-1])
        right_pos =  np.hstack((pos_sub[:,0].reshape(2,1), pos_sub[:,~left_mask][:,::-1]))

        right_min_dist = np.append(dist_expected[0], dist_expected[~left_mask][::-1])
        right_min_dist = np.maximum(2*right_min_dist[:-1], 1.0)

        left_diff = np.diff(left_pos, axis=1)
        left_dist = np.linalg.norm(left_diff, axis=0)
        left_corners = np.where(left_dist > left_min_dist)[0]

        right_diff = np.diff(right_pos, axis=1)
        right_dist = np.linalg.norm(right_diff, axis=0)
        right_corners = np.where(right_dist > right_min_dist)[0]

        return [left_corners, right_corners], [left_pos, right_pos], [left_angles, right_angles], [left_ranges, right_ranges], step
    ...
    def choose_setpoint(self, pos_sub, corners, angles_sub, ranges_sub):
        ...
        left_corners_2 = np.vstack((corners[0], np.zeros((corners[0].shape)))).astype(int)
        right_corners_2 = np.vstack((corners[1], np.ones((corners[1].shape)))).astype(int)
        corners_combined = np.hstack((left_corners_2, right_corners_2))
        # angles_combined = np.hstack((angles_sub[0], angles_sub[1]))
        # print(angles_combined)
        # input()

        for i in corners_combined.T:
            corner = pos_sub[i[1]][:, i[0]].reshape(2,1)
            corner_angle = angles_sub[i[1]][i[0]]

            print(i)

            if i[1] == 0:
                mask = angles_sub[i[1]] < corner_angle
            else:
                mask = angles_sub[i[1]] > corner_angle 
